...
export function reemplazarCanalActivo(canalIdBotonPulsadoEnModal, canalIdExistente) {
    try {
        ...
        if (divPadreACambiar) {
            let divExistenteACambiar = document.querySelector(
                `div[data-canal-cambio="${canalIdExistente}"]`,
            );
            let barraOverlayDeCanalACambiar = document.querySelector(
                `#overlay-de-canal-${canalIdExistente}`,
            );
            // evitar duplicados si canal que va a quedar de reemplazo ya existe en grid
            if (
                document.querySelector(`div[data-canal="${canalIdBotonPulsadoEnModal}"]`) &&
                divPadreACambiar !==
                    document.querySelector(`div[data-canal="${canalIdBotonPulsadoEnModal}"]`)
            ) {
        ...
        console.error(
            `Error intentar cambiar canal con id: ${canalIdExistente} por canal: ${canalIdBotonPulsadoEnModal}. Error: ${error}`,
        );
        mostrarToast(
            buildErrorToastMessage(
                t('errorReplaceChannel', {
                    oldChannel: canalIdExistente,
                    newChannel: canalIdBotonPulsadoEnModal,
                }),
                error,
            ),
            'danger',
